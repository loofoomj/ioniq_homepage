...
<%
	...
	SupportDTO sDto = (SupportDTO) sDao.freeboardMod(num);
%>
<body>
<%@ include file="../../templete/header.jsp"%>
<section>
	<div id="main">
		<form name="frm" action="supportModProcess.jsp?kind=<%=sDto.getKind() %>">
		<%if(sDto.getKind().equals("reple")){ %>
		<div id="title_write">댓글쓰기</div>
		<%}else if(sDto.getKind().equals("write")){ %>
		...
		<input type="hidden" value=<%=num %> name="num">
			<table>
				<tr>
					<td class="d_1">제목</td>
					<td><input class="inp" type="text" name="title" id="title" value="<%=sDto.getTitle() %>"></td>
				</tr>
				<tr>
					<td class="d_1">작성자</td>
			...
			<textarea rows="" cols="" name="contents" id="contents"><%=sDto.getContents() %></textarea>
			<input name="btn" id="btn_write" type="button" value="등록" class="btn">
		</form>
		<form action="../support.jsp?curPage=<%=curPage %>" method="post">
